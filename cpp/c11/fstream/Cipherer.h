...
bool decrypt(SG_ENCRYPT_TYPE type, const unsigned char *inputBuf, int inputLength, 
		     unsigned char **outputBuf, int& outputLength)
{
	cout << "Cipherer, decrypt [data] ..." << endl;
	if (!inputBuf || inputLength == 0)
	{
		cout << "Cipherer, decrypt[data]:parameters are invalid." << endl;
		return false;
	}
	...
	if ((type == SG_ENCRYPT_DES_CBC) && (mIvec.size() < 8))
	{
		cout << "Cipherer, decrypt[des cbc]:ivec < 8 characters:" << mIvec << endl;
		return false;
	}
	else if ((type == SG_ENCRYPT_AES_CBC) && ((mKey.size() < 32) || (mIvec.size() < 16)))
	{
		cout << "Cipherer, decrypt[aes cbc]:ivec < 16 characters or key < 32 characters." << endl;
		return false;
	}
	else if ((type == SG_ENCRYPT_AES_ECB) && (mKey.size() < 32))
	{
		cout << "Cipherer, decrypt[aes ecb]:key < 32 characters." << endl;
		return false;
	}
	...
	{
		case SG_ENCRYPT_DES_ECB:
			ret = EVP_DecryptInit_ex(ctx, EVP_des_ecb(), NULL, key, NULL);
			break;
		case SG_ENCRYPT_DES_CBC:
			ret = EVP_DecryptInit_ex(ctx, EVP_des_cbc(), NULL, key, ivec);
			break;
		case SG_ENCRYPT_AES_ECB:
		...
		case SG_ENCRYPT_AES_CBC:
			...
			break;
		default:
			cout << "decrypt type unknow, type:" << type << endl;
	}
	...
	*outputBuf = new unsigned char [inputLength + EVP_MAX_KEY_LENGTH];
	memset(*outputBuf, 0x00, inputLength + EVP_MAX_KEY_LENGTH);
	if (1 != EVP_DecryptUpdate(ctx, (unsigned char *)*outputBuf, &outputLength, inputBuf, inputLength))
	{
		cout << "Cipherer, failed to process decryption." << endl;
		EVP_CIPHER_CTX_free(ctx);
		delete []*outputBuf;
		return false;
	}
	...
	int finalOutputLength = 0;
	if (1 != EVP_DecryptFinal_ex(ctx, (unsigned char *)(*outputBuf + outputLength), &finalOutputLength))
	{
		cout << "Cipherer, failed to finalize decryption." << endl;
		delete []*outputBuf;
		EVP_CIPHER_CTX_free(ctx);
		return false;
	}
	...
	//clean the unuseful data
	memset(*outputBuf + outputLength, 0x00, inputLength + EVP_MAX_KEY_LENGTH - outputLength);
	cout << "Cipherer, Decrypt finished." << endl;
	return true;
}
